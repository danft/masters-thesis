...
% conter no mínimo 1 e no máximo 5 palavras-chave
\textoresumo[english]{
    Planar maximal covering with ellipses is an optimization problem which one wants to place
    ellipses on the plane to cover demand points, such that a function depending on the value of the
    covered points and on the cost of the ellipses that have been used is maximized.
    
    For the version of the problem where the ellipses are parallel to the coordinate axis, this work will
    present an algorithm based on previous works on the planar maximal covering with disks.
