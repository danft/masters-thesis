...
\tituloPT{Algoritmos para Problemas de Cobertura Máxima por Elipses}
\tituloEN{New Exact Algorithms for Planar Maximum Covering Location by Ellipses Problems}
\autor[Tedeschi, D.]{Danilo Tedeschi}
\genero{M} % Gênero do autor (M = Masculino / F = Feminino)
\orientador[Orientadora]{Profa. Dra.}{Marina Andretta}
%\coorientador{Prof. Dr.}{Fulano de Tal}
\curso{CCMC}
\data{02}{03}{2020} % Data do depósito
...
\textoresumo[english]{
   ...
   Besides finding optimal solutions for previously published instances, including the ones where no optimal solution was known, both algorithms proposed by us were able to obtain optimal solutions for some new larger instances having with up to seven hundred demand points and five ellipses.
    }{Optimization, Planar Maximal Covering Location Problem, Planar Maximum Covering Location by Ellipses, Geometric Computation}

\textoresumo[brazil]{
	Cobertura Máxima Planar por Ellipses é um problema de otimização em que deseja-se determinar o local para ellipses de forma fixa no plano para cobrir pontos de demanda para maximizar uma função que depende do valor dos pontos cobertos. 
Neste trabalho, propomos novos algoritmos exatos para duas versões desse problema, uma em que as ellipses tem que ser paralelas em relação aos eixos do sistema de coordenadas, e outro em que elas podem ser rotacionadas livremente.
Além de encontrarmos soluções ótimas para instâncias previamente publicadas, incluindo aquelas que nenhuma solução ótima era conhecida, ambos algoritmos propostos por este trabalho também foram capazes de determinar soluções ótimas para novas instâncias com até setecentos pontos e cinco ellipses.
	}{Otimização, Cobertura planar maximal por ellipses, Geometria Computacional}
...
%\label{chapter:pmclp}
%\input{tex/chapter/mcd.tex}


\chapter{Planar Maximum Covering Location by Ellipses Problem}
\label{chapter:mcd2}
\input{tex/chapter/mcd2.tex}


%\chapter{Maximum Covering by Ellipses}
%\label{chapter:mce}
%\input{tex/chapter/mce.tex}
...
\label{chapter:e3p}
\input{tex/chapter/e3p.tex}

\chapter{Planar Maximum Covering Location by Ellipses with Rotation Problem}
\label{chapter:mcer}
\input{tex/chapter/mcer.tex}

\chapter{Numerical Experiments}
\label{chapter:numerical}
\input{tex/chapter/numerical.tex}
...
\begin{apendicesenv}
	...
	\label{chapter:poly}
	\input{tex/appendix/poly.tex}
    
    \chapter{Tables of numerical results}
    \label{chapter:tables}
    \input{tex/chapter/tables.tex}
       
    %\chapter{Analytical solution to the 3 points ellipse problem}
    %\label{chapter:3pnts_ellipse_1}
    %\input{tex/appendix/3pnts_ellipse_1.tex}
