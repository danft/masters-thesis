...
Additionally, whenever we have an instance $(\Pp, \Ww, \{(a, b)\})$ with only one ellipse, we omit the index referring to the facility, and we use $\partial$ as the boundary operator, for example, given an instance of MCE, $\partial E_1(q_1)$ denotes an ellipse with shape parameters $(a_1, b_1)$ centered at $q_1$.
