...
This method seemed to work fine, but for some instances it did not find roots which were priorly known. This happened probably because of the transformation that was used to convert $\xi$ into a polynomial. It can be very numerically unstable as the tangent function grows to fast for angles that are greater than $\frac{\pi}{4}$. Depending on the instance, there could be roots very close to $0$ and others that are too far from it.
