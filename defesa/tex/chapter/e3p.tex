...
On \autoref{chapter:definitions} a brief introduction is given on how to get the roots of a polynomial. For that reason, we discuss two ways of converting $\xi$ into a polynomial. Symbolic computing was used to compute the polynomials, in practice an external Python library called SymPy was utilized (see \citeonline{sympy} for more details).
...
It is possible to show that with that substitution and changing the variable to $z=e^{it}$, we obtain the following function $g : \mathbb{S} \mapsto \mathbb{C}$:

\begin{equation}
g(z)=\sum_{k=0}^{12} c_k z^{k-6},
\end{equation}
for some $c_0, \dots, c_{12} \in \mathbb{C}$. It is important to notice that this definition of $g$ does not impose that $z$ stays in the unit circle as any complex number, for that the additional condition: $|z|=1$ would need to be added.
