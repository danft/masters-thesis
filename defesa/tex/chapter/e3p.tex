...
Initially, E3PNT is a problem with $3$ unknown variables: the two coordinates of the center point $q_x, q_y, $ and the angle of rotation $\theta$. In this section, we transform E3PNT into the problem of finding the roots of an univariate function using the known problem of determining the circumcenter of a triangle. We also show E3PNT has at most $6$ solutions.

To make the problem simpler, let us translate the system, so the point $u$ is at the origin. After that, we assume that the ellipse is actually axis-parallel and the points are the ones rotating. When an angle is found such that the three points lie on the border of the axis-parallel ellipse, a linear transformation can be applied to compress the x-axis by $\frac{b}{a}$, transforming the ellipse into a circle of radius $b$. This transformation can be seen on \autoref{fig:circumscribed-circle} where a solution of the E3PNT is transformed into a solution of the problem of finding a circumscribed circle of a triangle. 
This process can be parametrized by the angle of rotation of the points, as described by
...
\end{array}\right].
\end{equation}
Because of the invertibility of linear transformations, solutions for E3PNT can be obtained by reversing these transformations.
...
After that transformation, the problem to be solved is the one of checking the existence of a circumscribed circle of the triangle formed by the points $\{(0, 0), \varphi(v, \theta),\varphi(w, \theta)\}$ with radius $b$. Notice that the problems are indeed equivalent, which implies that a bijection between the set of solutions of each problem exists.
Working with this problem instead is preferred because of the reduction in the number of variables from three to one. This idea, however, would only be useful if checking the existence of a circumscribed circle with radius $b$ is a convenient problem.

It turns out that for three non-colinear points, there is always an unique circumscribed circle. Also, finding this unique circle can be done analytically, and its radius $R$ can be computed through the expression

\begin{equation}\label{eq:circumscribed_circle}
R = \dfrac{\norm{\varphi(v, \theta)}\norm{\varphi(w, \theta)}\norm{\varphi(v, \theta)-\varphi(w, \theta)}   }{4A(\theta)},
\end{equation}
with $A(\theta)$ being the area of the triangle formed by the points $\{(0, 0), \varphi(v, \theta), \varphi(w, \theta)\}$ (for more details about \autoref{eq:circumscribed_circle} see \citeonline[p.~189]{johnson1960}
). It is important to note that the transformation does not preserve distance or area; if that was true, the radius defined by \autoref{eq:circumscribed_circle} would be constant.

With the formula for the circumradius in hands, a function can be defined, such that its roots provide solutions for E3PNT.
...
Before continuing with this approach of trying to determine the roots of $\xi$, it is important to answer the question whether this set of roots is finite or not? Answering this question is also important because it automatically relates to the question about the number of solutions of E3PNT.


\subsection{The number of solutions of E3PNT}

One of the steps of the method developed in \autoref{chapter:ellipses_n} is to iterate over every solution of E3PNT for every triplet of points. Of course doing that is only possible for a finite number of solutions. Also, given that it is limited, discovering a bound for it is essential for determining the efficiency of the algorithm.
...
Going from $12$ solutions to $6$ is done by using the symmetry of ellipses. In \autoref{chapter:definitions}, it was stated that any rotation in the interval $[0, \pi)$ is identical to a rotation in $[\pi, 2\pi)$. Therefore, half of the roots of $\xi$ are in $[\pi, 2\pi)$ and can be ignored.
\end{proof}
...
The interpolation was run for several degrees for every triplet of points present in an instance from \citeonline{canbolat}. We considered the error to be $|\xi(\hat{\theta})|$ for every root $\hat{\theta}$ found.
...
Note that the the square root of $\hat{y}$ does not need to be calculated, as only the angles are needed and they can be obtained by the following identity
\begin{equation}
angle(\sqrt{z}) = angle(z)/2.
\end{equation}
Finally, using the QR algorithm mentioned in \autoref{chapter:definitions}, a $\bigO(n^3)$ algorithm can be constructed for E3PNT. Because $n$ will always be $6$, we can say that we have a $\bigO(1)$ algorithm that takes around $6^3$ operations.
...
c_k = \overline{c_{6-k}},
\end{equation}
for $k=0, \dots, 6$. For now, we neither have any ideas on how \autoref{eq:palindromic_pol} could be proved nor how it could be used for further improving our algorithm.

Assuming that an algorithm to obtain the eigenvalues of a matrix, the following algorithm to obtain every solution of E3PNT is described as follows.
...
\begin{algoritmo}
	...
	\begin{algorithmic}[1]
		...
		\For{$j\in\{0, \dots, 5\}$}
			\State $A_{6,j+1} \gets -\frac{c_j}{c_6}$
		\EndFor
		
		\State $Q \gets \{\}$
		
		\ForAll{$q \in eig(A)$} \Comment{$eig(A)$ returns every eigenvalue of $A$.}
		\State $\theta\gets angle(q)$
		\If{$|\theta|=1$}
			\State $Q \gets Q \cup \{\theta\}$
		\EndIf
		\EndFor
		
		\State $Sols\gets\{\}$
		\ForAll{$\theta\in Q$}
		\State Let $c$ be the center of the circumscribe circle of the points $(0,0); \varphi(\hat{v}, \theta); \varphi(\hat{w}, \theta)$.
		\State $Sols \gets \cup \{(\theta, \varphi^{-1}(c, \theta)+u)\}$
		\EndFor
