...
Let $E$ be the coverage area of an ellipse with the given shape parameters $(a, b)$, then a solution of E3PNT is a pair $(q, \theta) \in \R^2\times[0, \pi)$, such that $\{u, v, w\} \subset \partial E(q, \theta)$. As a last remark, because of its application on \autoref{chapter:ellipses_n}, it is only useful a method would only be useful to our case if it can encounter every solution of E3PNT. This requirement, of course, makes the problem more challenging.

\section{Transforming the problem}

Initially, E3PNT is a problem with $3$ unknown variables: $q_x, q_y, $ and $\theta$. In this section, we transform E3PNT into the problem of finding the roots of an univariate function using the known problem of determining the circumcenter of a triangle. We also shown that the number of solutions of E3PNT is at most $6$.
