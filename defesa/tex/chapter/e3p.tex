In this chapter, we introduce the problem of determining every center and angle of rotation of an ellipse with fixed shape parameters, such that it contains three given points.
...
We call the problem of finding a center and an angle of rotation for an ellipse given its shape parameters and three points that have to be on its border \sigla{E3P}{Ellipse by Three Points}. An instance of it is given by three points $u, v, w \in \R^2$ and the ellipse's shape parameters $(a, b) \in \R^2_{>0}$, with $a > b$.
...
\begin{figure}
	...
	\includegraphics[scale=.43]{tex/figures/circumscribed-circle}
	\fautor
...
The interpolation was run for several degrees, for every triplet of points in an instance from \citeonline{canbolat}. We considered the error to be $|\xi(\hat{\theta})|$ for every root $\hat{\theta}$ found.

The results are shown in \autoref{fig:interpolant_error}, where the worst error found among every root is plotted against the interpolant degree. To improve accuracy while keeping the interpolation degree low, we took a suggestion from \citeonline{boyd:2013}, and divided  $[0, \pi)$ into $K$ sub-intervals. That way, $K$ interpolations are used to obtain every root in the whole interval. It can be seen in \autoref{fig:interpolant_error} that this strategy helps a lot in improving the accuracy even when degrees around $8$ are used.

\begin{figure}
	\centering
	\caption{The interpolation error measured on roots that were found.}
	\includegraphics[scale=.84]{tex/figures/interpolant_error}
	\fautor
...
\end{equation}
for some $c_0, \dots, c_{12} \in \mathbb{C}$. As the equalities on \autoref{eq:complex_trig} are valid for any $\theta \in \R$, function $g(e^{i\theta})$ and $\xi$ are equivalent, since $g(e^{i\theta}) = \xi(\theta)$ for any $\theta \in [0, 2\pi)$. 
...
Computing the coefficients of the complex polynomial $h$ is done in our implementation using a symbolic computation library for Python called Sympy (for more information about it see \citeonline{sympy}). This way, $c_0, \dots, c_{12}$ can be written as functions of the E3P's instance. Then, basically, after building a companion matrix for the polynomial $h$, \autoref{algoritmo:e3p}, for every eigenvalue, applies the reverse transformations described by \autoref{eq:trpnts} to obtain a solution for E3P.
