...
\subsection{The circumradius problem}

After that transformation, the problem to be solved is the one of checking the existence of a circumscribed circle of the triangle formed by the points $\{(0, 0), \varphi(v, \theta),\varphi(w, \theta)\}$ with radius $b$. 
Note that the problems are indeed equivalent: $\varphi$ is a linear function, thus its inverse $\varphi^{-1}$ is well defined and can be used to determine a bijection between the set of solutions of both problems.
