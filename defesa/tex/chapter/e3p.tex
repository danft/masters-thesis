...
Assuming that an algorithm to obtain the eigenvalues of a matrix, the following algorithm to obtain every solution of E3PNT is described in \autoref{algoritmo:e3p}.
...
\begin{algoritmo}
	...
	\begin{algorithmic}[1]
		...
		\For{$i\in\{1, \dots, 6\}$}\Comment{Constructing the companion matrix.}
			...
			\State $A_{6,i} \gets -\dfrac{c_{i-1}}{c_6}$
		\EndFor
		...
		\State $Sols \gets \cup \{(\varphi^{-1}(c, \theta)+u, \theta)\}$
		\EndFor
