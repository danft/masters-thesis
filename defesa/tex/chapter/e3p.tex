...
We refer to this problem as \sigla{E3PNT}{Ellipse by Three points}, and an instance of it is given by three points $u, v, w \in \R^2$ and $E$, an ellipse with shape parameters $(a, b) \in \R^2_{>0}$, with $a > b$. A solution of E3PNT is a pair $(q, \theta) \in \R^2\times[0, \pi)$, such that $\{u, v, w\} \subset \tilde{E}(q, \theta)$.
...
It is possible to make another reduction to obtain a degree-$6$ polynomial, lowering in half the number of rows of the companion matrix utilized on obtaining the roots of $h$. 
