...
\begin{algoritmo}
	...
	\begin{algorithmic}[1]
		...
		\Ensure{An optimal solution of MCER.}
		
		\item[]
		
		\Procedure{$MCER$}{$\Pp, \Ww, \Rr, j=1$}
		\If{$j = m+1$}
		\State \Return $0$
		\EndIf
...
%		\State $((q_{j+1},\theta_{j+1}); \dots; (q_m, \theta_m)) \gets  MCE(\Pp\setminus Cov, \Ww, \Rr, j+1)\}$
...
\begin{proof}
	...
	Unless $Z=\Pp$, when choosing the center and angle of rotation for the $j$-th ellipse, \autoref{algoritmo:mcer} does not consider $(q_j, \theta_j) \in S_j$, such that $\Pp \cap E_j(q_j, \theta_j) \subset Z$. Apart from those solutions, which are non-optimal, \autoref{algoritmo:mcer} considers every solution in $\Omega(\Pp, \Ww, \Rr)$.
\end{proof}
