...
\begin{definicao}\label{def:feasible_angle}
	Let $E$ be the coverage region of an ellipse and $u, v \in \R^2$. An angle $\theta \in [0, \pi)$ is said to be $(E, u, v)$-feasible if there is $q \in \R^2$ such that $\{u, v\} \subset \partial E(q, \theta)$. In addition to that, given an instance of MCER, we refer to the set of $(E_j, u, v)$-feasible angles as 
...
Let $(Q^*, \Theta^*)$ be an optimal solution of an instance $(\Pp, \Ww, \Rr)$ of MCER. Then, for any $E_j \in \E$, such that $|\Pp \cap E_j(q_j^*, \theta_j^*)|>2$, at least one of the two cases is true:
	...
	\begin{enumerate}[label=(\alph*)]
		\item There is an equivalent solution $(Q', \Theta')$, such that $|\Pp\cap \partial E_j(q'_j, \theta'_j)| \ge 3$.\label{lema:3pnts_a}
...
\begin{proof}
	...
	According to \autoref{lema:mce_2b}, there exists $\{u, v\} \subset \Pp$, such that an equivalent optimal solution exists with $u, v$ on the border of $E_j$. 
