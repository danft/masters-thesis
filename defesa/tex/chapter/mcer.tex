...
For every ellipse in an instance, unless an equivalent optimal solution with three points on its border exits, the angle of rotation can practically be ignored. That is what \autoref{lema:3pnts} is saying, that the implication of a non-existent equivalent optimal solution with three points on the ellipse's border implies that any feasible angle can be used. Therefore, we introduce a corollary next which describes a way of constructing a CLS that contains at least one optimal solution.

\begin{corolario}
	Let $(\Pp, \Ww, \Rr)$ be an instance of MCER. We define for all $j\in\{1, \dots, m\}$, $\Gamma_j^{(1)}, \Gamma_j^{(2)}, $and $\Gamma_j^{(3)}$ as
	\begin{align*}
	\Gamma_j^{(1)} &= \bigcup_{u \in \Pp} \{(u, 0)\in \R^2\times\R\}\\
	\Gamma_j^{(2)} &= \bigcup_{\{u, v\} \subset \Pp} \{(q, angle(u,v))\in \R^2\times\R: \{u, v ,w\} \subset \partial E_j(q, angle(u,v))\}\\
	\Gamma_j^{(3)} &= \bigcup_{\{u, v, w\} \subset \Pp} \{(q, \theta)\in \R^2\times\R: \{u, v ,w\} \subset \partial E_j(q, \theta)\}.
	\end{align*}
