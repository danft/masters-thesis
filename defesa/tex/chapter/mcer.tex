...
\begin{proof}
	...
	Moreover, in \autoref{section:ellipses_intersection}, we discuss the problem of determining the intersections between two ellipses. This problem is equivalent to the problem of determining every center and angle of rotation that puts two points on the border of an ellipse, which is the problem used in the definition of $S_j^{(2)}$ for every pair of points. In \autoref{section:ellipses_intersection}, it is shown that this problem has at most two solutions. Therefore, we have that $|S_j^{(2)}| = 2\binom{n}{2}$.
	
	For the last case, we are left with solutions where the $j$-th ellipse covers more than two points, and there exists an equivalent solution with three points on its border. 
	As $S_j^{(3)}$ contains every center and angle of rotation that puts three points on the border of the $j$-th ellipse, an equivalent solution for this case is present in the set of solutions $\Omega(\Pp, \Ww, \Rr)$. Also, in \autoref{lema:3pnts} it is stated that the number of centers and angles of rotation that make an ellipse have three given points on its border is at most six. Therefore, we have that $|S_j^{(3)}| = 6\binom{n}{3}$.
...
Let $E$ be the coverage region of an ellipse with shape parameters $(a, b)$. We assume that in \autoref{algoritmo:mcer1}, the procedure $e2p(u, v, a, b)$ returns every $(q, \tilde{\Phi}_j(u,v)) \in \R^2\times\R$, such that, $\{u, v\}\subset \partial E(q, \tilde{\Phi}_j(u,v))$. This procedure can be defined using the results of \autoref{section:ellipses_intersection}.
