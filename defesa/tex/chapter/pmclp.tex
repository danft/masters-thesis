...
As this work is developed torwards an exact method for MCD, the focus is on constructing a CLS that contains an optimal solution.
This is done in \citeonline{church:1984} where a circle is defined for every demand point and a \sigla{CIPS}{circle intersection point set} is constructed containing every pairwise intersection of circles and the demand points. 
...
This version of the problem will be refered to as \sigla{MCD-1}{Maximum Covering by One Unit Disk} and it is just a specific case of MCD with one unit disk ($m=1$ and $r_1=1$). It is treated before the case with multiple facilities because it is used to construct a CLS that is guaranteed to contain an optimal solution.
...
In \citeonline{drezner}, the main idea used to develop the $\bigO(n^2\log{n})$ algorithm is that, even though there are infinitely many points where the disk could be placed, only a few of them, a finite amount of $\bigO(n^2)$, needs to be considered for the method to find an optimal one--this is also the idea used to construct the CIPS in \citeonline{church:1984}.
...
In \citeonline{chazelle:1986} and \citeonline{cabello:2006}, on the other hand, instead of working directly with MCD-1, an equivalent problem called Maximum Weight Clique was introduced. The algorithm that we later describe in this work also uses this equivalence and it is described next.

\section{Maximum Weight Clique Problem}

Let $\Pp=\{p_1,\dots,p_n\}$, with $p_i \in \R^2$, be a set of points; $\D=\{D_1,\dots,D_n\}$ a set of unit disks, such that $D_i$ is centered at $p_i$, $i=1,\dots,n$, with every disk having a weight $w_i > 0$, $i=1,\dots,n$. A clique, in this context, is a non-empty intersection area of two or more disks. A solution for the problem is a clique.


Note that this is different than the clique problem on a intersection graph (a graph where the vertices are the disks and an edge exists if there is an intersection between two disks). 


As shown in \autoref{fig:three_disks_no_intersection}, three disks could have non-empty pairwise intersection (which qualifies them as a clique on the intersection graph), however, the intersection of all the three together still be empty. 
That is why the clique problem for unit disks is also referred to as the Maximum Geometric Clique Problem and as the Maximum Graphical Clique Problem when there is only the pairwise intersection condition \cite{inplace:2014}. 
