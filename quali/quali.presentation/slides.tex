...
\begin{frame}{Maximum Weight Clique Problem}{Multiple disks}
	\begin{itemize}
		\item In \cite{cabello:2006} a $\bigO(n^{2m-1})$ algorithm was proposed. Also a $(1-\epsilon)-$approximation that runs in $\bigO(n\log{n})$ was introduced.
		
		\item In \cite{zhou} a heuristic method using an algorithm called mean-shift was developed. The mean-shift algorithm converges to a local density maxima of any probability distribution and it is used to find a smaller candidate list of centers for the disks.
	\end{itemize}
...
\begin{frame}{Maximal Covering by Ellipses}{$m$ ellipses}
	...
	\begin{itemize}
		...
		\item \cite{canbolat} is the very first study on the problem. Slow exact method, a heuristic one was proposed.
		\item \cite{andreta} proposed a method that breaks the problem into smaller optimization ones. Also, they developed a method for the non-axis-parallel case.
	\end{itemize}
\end{frame}
...
\begin{frame}{Maximal Covering by Ellipses}
	...
	\begin{itemize}
		...
		\item Simpler than the $m$ disks algorithm proposed by \cite{cabello:2006}. Achieves a similar complexity ($\bigO(n^{2m-1})$).
